...
export default function PokemonError({ error, reset }: PokemonErrorProps) {
  return (
    <div className="flex min-h-[60vh] items-center justify-center px-4 py-16">
      <div className="w-full max-w-xl rounded-3xl border border-red-500/20 bg-slate-900/60 p-8 text-center text-white shadow-2xl backdrop-blur-md shadow-red-950/5 relative overflow-hidden">
        ...
        <p className="mt-3 text-sm leading-6 text-slate-400 max-w-md mx-auto">
          No pudimos conectar con la base de datos de Pokédex. Por favor, intenta de nuevo o regresa al listado.
        </p>

        {/* Tech Specs block */}
        <div className="mt-6 rounded-2xl border border-white/5 bg-slate-950/60 p-4 text-left text-xs font-mono text-slate-400">
          <span className="block font-bold text-red-400 mb-1 uppercase tracking-wider text-[10px]">Detalle técnico</span>
          <span className="block break-words">{error.message || "Error de red desconocido al consultar PokeAPI"}</span>
        </div>
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
          <button
            type="button"
            onClick={reset}
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-red-500 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-red-950/20 hover:bg-red-600 active:scale-95 transition cursor-pointer"
          >
            <IoRefresh size={18} />
            Reintentar
          </button>
